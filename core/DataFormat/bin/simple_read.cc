...
int main(int argc, char** argv){
  ...
  my_storage.set_verbosity(foolight::msg::kDEBUG);
  
  // Step 0: Set I/O mode: we are reading in, so "READ"
  my_storage.set_io_mode(foolight::storage_manager::kREAD);
  ...
  while(my_storage.next_event()){
    ...
    auto my_track_v = my_storage.get_data<foolight::event_track>("test");
    //auto my_track_v = (::foolight::event_track*)(my_storage.get_data(::foolight::data::kTrack,"test"));
    ...
    std::cout 
      << Form("Found event %d run %d subrun %d ... %zu tracks! ", 
	      my_track_v->event_id(),
	      my_track_v->run(),
	      my_track_v->subrun(),
	      my_track_v->size())
